...
public class LinearRegression {
    ...
    private final double r2_adj;
    private final double t1_stat;
     ...
     */
    public LinearRegression(double[] x, double[] y, subSet set) {
        if (x.length != y.length) {
            throw new IllegalArgumentException("array lengths are not equal");
        }
        int nn = set.indexes.size();
        int n = 0;
        // first pass
        double sumx = 0.0, sumy = 0.0, sumx2 = 0.0;
        for (int j = 0; j < nn; j++) {
            int i = set.indexes.get(j);
            if (x[i] * y[i] == x[i] * y[i]) {
                sumx += x[i];
                sumx2 += x[i] * x[i];
                sumy += y[i];
        ...
        t1_stat = Math.abs(slope / Math.sqrt(svar1));
    }
    ...
    public double R2_adj() {
        return r2_adj;
    }

    public double T1_stat() {
        return t1_stat;
    }
    ...
    public String toString() {
        ...
        s.append("  (R_adj^2 = " + String.format("%.4f", r2_adj) + ")");
        s.append("  (t1_stat = " + String.format("%.4f", t1_stat) + ")");
